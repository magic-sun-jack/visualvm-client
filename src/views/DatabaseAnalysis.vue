<template>
  <div class="p-4 space-y-6">
    ...
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold text-foreground">数据库调用分析</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <Card>
        <CardContent class="p-6 text-center">
          <p class="text-sm text-muted-foreground">慢查询数量</p>
          ...
          <table class="min-w-full divide-y divide-border">
            ...
            <tbody class="bg-background divide-y divide-border">
              <tr v-for="call in filteredCalls" :key="call.id" class="hover:bg-muted/50">
                ...
                <td class="px-6 py-4">
                  <div class="max-w-xs truncate text-sm text-foreground" :title="call.sql">
                    {{ call.sql }}
                  </div>
                </td>
...
// 模拟数据
const totalCalls = ref(1250)
const successCalls = ref(1180)
const averageExecutionTime = ref(45)
const slowQueryCount = ref(23)

const databaseCalls = ref([
  {
    id: '1',
